...
6. In the resulting window, copy and paste the below deployment yaml into it and click **Create**
  ...
  7. Here is a quick explanation of the deployment yaml
     ...
     - This deployment has 1 pod which contains 2 containers:
         ...
         - This container uses the same volume as initContainer `llama-models` for underlying storage and hence can access the model downloaded by initContiner. Lines 43-46 specifies the PVC (Persistent Volume Claim) used as the source of storage for the volume named `llama-models`. Recall that we created this PVC at the beginning of this lab!
